...
arma::vec compute_stat(arma::vec x, int T, String type, arma::vec weight) {
  ...
  if (type == "decoupled_snemk") {
    ...
    for (int k = T + 1; k <= N; k++) {
      ...
      // 【核心改动 1：初始化独立的分子最大值和分母最小值】
      double max_num = -1.0; 
      double min_denom = 1e300; // 初始化为一个极大的数 (Infinity)
      
      for (int z = T; z < k; z++) {
        
        // 1. 极速计算右侧样本自正则矩阵 V_R(z,k)
        double V_R = 0.0;
        if (k - 1 >= z + 1) {
          double sum_1 = (k - 1) - z;
        ...
        // 3. 独立计算并更新分母最小值 (加入兜底保护)
        double v_total = V_L[z] + V_R;
        if (v_total > 1e-12) {
          double denom = std::sqrt(v_total);
          if (denom < min_denom) {
            min_denom = denom;
          }
        }
      ...
      // 【核心改动 2：仅在循环外部做一次除法】
      // 只要 min_denom 被更新过且不为 0，即计算当前时刻 k 的最终统计量
      if (min_denom > 1e-12 && min_denom < 1e299) {
        stat_val(k - T - 1) = (max_num / min_denom) * weight(k - T - 1);
      } else {
        stat_val(k - T - 1) = 0.0;
      }
    }
